...
test('Miami structures leave the full racing corridor clear and stay within geometry budget', () => {
  ...
  const track = { id: 'miami', points: [[0,3,0],[0,3,230],[35,3,380],[180,4,420],
    [320,5,360],[340,6,200],[270,6,80],[340,4,-80],[210,3,-200],[40,3,-170],[-80,3,-70]] } as Track;
  const circuit = new Circuit(track), root = new T.Group(), collision = new CollisionWorld();
  ...
  for (const solid of collision.solids) {
    ...
    for(let i=0;i<p.count;i++) minimum=Math.min(minimum,circuit.terrainAt(p.getX(i),p.getZ(i)).distance);
  }
  ...
  kit.update(12,true); expect(boat.position.y).not.toEqual(boat.userData.baseY);
  kit.update(12,false); expect(boat.position.y).toEqual(boat.userData.baseY);
  console.info(JSON.stringify({miamiGeometry:kit.metrics,minimumCollisionRoadClearance:minimum}));
  collision.dispose();
});
