...
describe('verified source findings', () => {
  ...
  it('duplicate finish and saved-result replay cannot pay a second purse', () => {
    const s = freshSave();
    const result = { id: 'test-race-001', place: 1, chapter: 0, style: 400, difficulty: 'easy' as const };
  ...
  for (const cameraZ of [-12, -0.35]) for (const heading of [0, 0.8, 2.7, -2.9]) {
    it(`keyboard/touch/controller right is screen-right at heading ${heading}, camera ${cameraZ}`, () => {
      ...
      const c = new PerspectiveCamera(60, 1.6, .05, 1000);
      c.position.copy(forward).multiplyScalar(cameraZ);c.position.y=1;
      c.lookAt(forward.clone().multiplyScalar(20));c.updateMatrixWorld();
      const center=forward.clone().multiplyScalar(20);
      for (const value of [steeringToLane(false,true),steeringToLane(false,false,1)]) {
        expect(center.clone().addScaledVector(lateral,value).project(c).x).toBeGreaterThan(center.clone().project(c).x);
        // +Z-front model: negative yaw points the front wheels to screen right.
        expect(value*.35).toBeLessThan(0);
      }
    });
  }
});
